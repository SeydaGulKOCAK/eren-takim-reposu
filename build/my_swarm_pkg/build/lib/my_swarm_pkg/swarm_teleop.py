...
class SwarmTeleopNode(Node):
    ...
    def _publish_loop(self) -> None:
        ...
        # execute_at: Wi-Fi gecikme kompansasyonu (now + 50ms)
        now_ns   = self.get_clock().now().nanoseconds
        delay_ns = int(CMD_DELAY_MS * 1_000_000)
        from rclpy.time import Time as _RclpyTime
        msg.execute_at = _RclpyTime(nanoseconds=now_ns + delay_ns).to_msg()
    ...
    def _keyboard_loop(self) -> None:
        ...
        try:
            ...
            while rclpy.ok():
                ...
                # ── Mod geçişi ────────────────────────────────────────────────
                # ESC → IDLE (q artık sadece "aşağı in" — çakışma giderildi)
                if ch == '\x1b':  # ESC → IDLE
