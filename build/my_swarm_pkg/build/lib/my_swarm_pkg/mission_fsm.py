...
class MissionFSMNode(Node):
    ...
    def _set_altitude_interactive(self) -> None:
        """Jüri'nin belirteceği uçuş irtifasını ayarla ve intent_coordinator'a ilet."""
        print(f'\n  Mevcut irtifa: {_BOLD}{self._gcs_altitude:.1f}m{_RESET}')
        print(f'  Yeni irtifa (metre, örn: 15): ', end='', flush=True)
        try:
            val = input('').strip()
            alt = float(val)
            if alt < 2.0 or alt > 100.0:
                print(f'  {_YELLOW}⚠  Geçersiz değer (2-100m arası olmalı){_RESET}')
                return
            self._gcs_altitude = alt
            msg = Float64()
            msg.data = alt
            self._altitude_pub.publish(msg)
            print(f'  {_GREEN}✅ İrtifa {alt:.1f}m olarak ayarlandı ve gönderildi{_RESET}')
        except (ValueError, EOFError):
            print(f'  {_YELLOW}⚠  Geçersiz giriş{_RESET}')
    ...
    def _print_help(self) -> None:
        print(
            ...
            f'\n  {_BOLD}h{_RESET} / alt     → Uçuş irtifası gir (jüri söyler)'
            f'\n  {_BOLD}m{_RESET} / map     → QR koordinatı gir (jüri söyler)'
        ...
        try:
            while rclpy.ok():
                ...
                if cmd in ('s', 'start'):
                    ...
                    self.setup_qr_map()
                elif cmd in ('h', 'alt', 'altitude', 'irtifa'):
                    self._set_altitude_interactive()
