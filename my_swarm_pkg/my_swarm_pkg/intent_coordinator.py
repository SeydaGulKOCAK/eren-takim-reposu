...
class IntentCoordinator(Node):
    ...
    def __init__(self):
        ...
        self._qr_map_ready:  bool = False  # SetQRMap servisi çağrılmadan TASK1 başlamaz
        self._qr_map_nodes:  dict = {}     # QR1 pozisyonunu task_trigger'da kullanmak için
        self._load_local_qr_map()          # YAML'dan QR konumlarını ön-yükle
        ...
        # 2c) GCS'den gelen irtifa komutu (jüri yarışma günü söyler)
        self.create_subscription(
            Float64, '/gcs/drone_altitude', self._on_gcs_altitude, 10
        )
    ...
    def _load_local_qr_map(self):
        """qr_map.yaml'dan QR node pozisyonlarını yükle (task_trigger'da QR1'e navigasyon için)."""
        try:
            try:
                from ament_index_python.packages import get_package_share_directory
                pkg = get_package_share_directory('my_swarm_pkg')
                path = os.path.join(pkg, 'config', 'qr_map.yaml')
            except Exception:
                path = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'qr_map.yaml')
            ...
            self.get_logger().warn(f'[{self.ns}] QR map ön-yükleme başarısız: {e}')

    def _on_gcs_altitude(self, msg: Float64):
        """GCS'den gelen uçuş irtifasını güncelle (jüri yarışma günü söyler)."""
        if 2.0 <= msg.data <= 100.0:
            self._drone_altitude = msg.data
            self.get_logger().info(
                f'[{self.ns}] ✈️  Uçuş irtifası güncellendi: {msg.data:.1f}m (GCS)'
            )
    ...
    def _on_task_trigger(self, msg: TaskTrigger):
        ...
        if msg.start:
            ...
            # QR1 pozisyonunu YAML'dan ön-yükle (ARMING biter bitmez kullanılacak)
            qr1 = self._qr_map_nodes.get(1, self._qr_map_nodes.get('1', {}))
            if qr1:
                pos = qr1.get('position', {})
                self._target_pos = Point(
                    x=float(pos.get('x', 0.0)),
                    y=float(pos.get('y', 0.0)),
                    z=0.0,
                )
                self._target_yaw = self._compute_yaw_to(self._target_pos)
                self.get_logger().info(
                    f'[{self.ns}] 🎯 İlk hedef QR1 @ '
                    f'({self._target_pos.x:.1f}, {self._target_pos.y:.1f}) yüklendi'
                )
    ...
    def _on_qr_result(self, msg: QRResult):
        ...
        if msg.next_qr_id == 0:
            ...
            self._target_pos    = msg.next_qr_position   # SONRAKI QR konumu (bug fix: qr_position değil!)
            self._target_yaw    = self._compute_yaw_to(msg.next_qr_position)
            self._mission_phase = MissionPhase.NAVIGATING
            self.get_logger().info(
                f'➡️  [{self.ns}] Sonraki hedef: QR#{self._current_qr_id} '
                f'@ ({msg.next_qr_position.x:.1f}, {msg.next_qr_position.y:.1f}m) '
